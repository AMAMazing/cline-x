...
fast_mode = str(config.get('fast_mode', 'False')).lower() == 'true'
...
@app.route('/fast-mode', methods=['POST'])
@limiter.exempt
def set_fast_mode():
    global fast_mode, config
    try:
        data = request.get_json()
        ...
        fast_mode = new_state
        config['fast_mode'] = str(fast_mode)
        write_config(config)
        logger.info(f"Fast mode set to: {fast_mode}")
        return jsonify({'success': True, 'fast_mode': fast_mode})
    except Exception as e:
        logger.error(f"Error setting fast mode: {str(e)}")
        return jsonify({'success': False, 'error': str(e)}), 500
